...
//semaphores
sem_t triage_queue_full;
sem_t triage_queue_empty;
sem_t doctor_queue_full;
sem_t doctor_queue_empty;
...
void init_simulation(uint32_t np)
{
   ...
   //init semaphores
   sem_init(&triage_queue_full,0,0);
   sem_init(&triage_queue_empty,0,FIFO_MAXSIZE);
   sem_init(&doctor_queue_full,0,0);
   sem_init(&doctor_queue_empty,0,FIFO_MAXSIZE);
...
{
   while(1){
      //wait for triage queue to have a space
      sem_wait(&triage_queue_full);
      ...
      //waits for doctor queue to have a space
      sem_wait(&doctor_queue_empty);
      printf("\e[34;01mNurse: add patient %u with priority %u to doctor queue\e[0m\n", patient, priority);
      insert_pfifo(&hd->doctor_queue, patient, priority);      
      sem_post(&doctor_queue_full);

      sem_post(&triage_queue_empty);
   }
}
...
int main(int argc, char *argv[])
{
   ...
   //Thread creation
   pthread_t thr_docs[ndoctors];
   pthread_t thr_nurses[nnurses];
   pthread_t thr_patients[npatients];

   for(uint32_t i=0 ; i<ndoctors; i++){
      thread_create(thr_docs + i, NULL, doctor_iteration, NULL);
   }
   for(uint32_t i=0 ; i<nnurses; i++){
      thread_create(thr_nurses + i, NULL, nurse_iteration, NULL);
   }
   for(uint32_t i=0 ; i<npatients; i++){
      int* a = (int *)malloc(sizeof(int));
      *a = i;
      thread_create(thr_patients + i, NULL, patient_life, a);
   }

   for(uint32_t i=0 ; i<ndoctors; i++){
      thread_join(thr_docs[i], NULL);
   }
   for(uint32_t i=0 ; i<nnurses; i++){
      thread_join(thr_nurses[i], NULL);
   }
   for(uint32_t i=0 ; i<npatients; i++){
      thread_join(thr_patients[i], NULL);
   }

   sem_destroy(&triage_queue_full);
   sem_destroy(&triage_queue_empty);
   sem_destroy(&doctor_queue_full);
   sem_destroy(&doctor_queue_empty);
